...
const NotificationsScreen = () => {
  ...
  const fetchNotifications = async (pageNum = 1, isRefresh = false) => {
    try {
      const response = await axios.get(
        `http://your-api-url/api/notifications/user/${userId}?page=${pageNum}&limit=20`
      );
  ...
  const markAsRead = async (notificationId) => {
    try {
      await axios.patch(
        `http://your-api-url/api/notifications/${notificationId}/read`
      );
  ...
  const markAllAsRead = async () => {
    try {
      await axios.patch(
        `http://your-api-url/api/notifications/user/${userId}/read-all`
      );
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await axios.delete(
                `http://your-api-url/api/notifications/${notificationId}`
              );
