...
const AdminTaskAssignmentGlobal = ({ EmployeType = 'company-employee' }) => {
  ...
  // Users fetch based on type
  const { data: users, isLoading: usersLoading } = useQuery({
    queryKey: ['users', EmployeType],
    queryFn: async () => {
      const response = await axios.get(`/auth/byrole/${EmployeType}`);
  ...
  const setTargetMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['allTargets', EmployeType]);
  ...
  // All targets for this user type
  const { data: allTargets } = useQuery({
    queryKey: ['allTargets', EmployeType],
    queryFn: async () => {
      const response = await axios.get(`/targets/all-targets?role=${EmployeType}`);
...
export default AdminTaskAssignmentGlobal;